...
const enVia = (ubicacion, ruta, tipo) => {
	if (utils.containsLocation(ubicacion, ruta.area[datos.area], true)) {
...
const nivel2 = () => {
	Object.keys(proceso.pickups).forEach((idRO) => {
		Object.keys(proceso.dropoffs).forEach((idRD) => {
			if (idRO != idRD) {
				let cruzado = cruze(datos.rutas[datos.rutas.findIndex(x => x.id == idRO)].path,
					datos.rutas[datos.rutas.findIndex(x => x.id == idRD)].path,
				...
				if (cruzado.fin.length > 0 && cruzado.ini.length > 0) {
					let opciones = recorrerCruze(
						...
						datos.rutas[datos.rutas.findIndex(x => x.id == idRO)],
		...
		datos.rutas.forEach(idR => {
			if (idRO !== idR.id) {
				let cruzeInicial = cruze(
					datos.rutas[datos.rutas.findIndex(x => x.id == idRO)].path,
					idR.path,
					proceso.pickups[idRO][0],
					idR.path.length - 1);
		...
		for (let indexFin = 0; indexFin < fin.length; indexFin++) {
			if (ini[indexIni] < fin[indexFin]) {
				...
				respuesta.push({
					...
					score:
						(origen != null ? utils.computeDistanceBetween(origen, R[ini[indexIni]]) *
							datos.config.proportion : 0) +
						D +
						(destino != null ? utils.computeDistanceBetween(destino, R[fin[indexFin]]) *
							datos.config.proportion : 0)
				});
			}
		}
...
const verificarProceso = () => {
	...
	if (proceso.options.length > 0) {
		...
		proceso.options.forEach((opcion) => {
			...
			opcion.startWalk = utils.computeDistanceBetween(datos.origen, rutas[0].startPoint);
			opcion.startWalkDistance = opcion.startWalk > 1000 ? (opcion.startWalk / 1000).toFixed(1) + "km" :
				(opcion.startWalk).toFixed(1) + "m";

			//-------------------
			opcion.endWalk = utils.computeDistanceBetween(datos.destino, rutas[rutas.length - 1].endPoint);
 ...
 */
const cruze = (ruta1, ruta2, ini, fin) => {
	let m1 = { fin: [], ini: [] };
	for (let r1i = ini + 1; r1i < ruta1.length; r1i++) {
		for (let r2i = fin; r2i >= 0; r2i--) {
			let da = utils.computeDistanceBetween(ruta1[r1i], ruta2[r2i]);
			let ds;
			if (r2i - 1 >= 0) {
				ds = utils.computeDistanceBetween(ruta1[r1i], ruta2[r2i - 1]);
			}
			if (da <= datos.config.walkingDistance && (ds ? (da < ds) : true)) {
	...
	if (m1.fin.length > 0) {
		...
		const add2CG = (index, number) => {
			cg.push({
				ind: index,
				dist: utils.computeDistanceBetween(ruta1[m1.fin[index]], ruta2[m1.ini[index]]),
				num: number
			});
		}
